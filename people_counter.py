	...
	while True:
		...
		for (objectID, centroid) in objects.items():
			...
			# to determine direction
				...
				if not to.counted:
					# UP direction -> Enter: moving upwards (direction < 0) and in upper half
					if direction < 0 and centroid[1] < (H // 2) and max_y >= (H // 2 - 20):
						totalDown += 1
					...
					# DOWN direction -> Exit: moving downwards (direction > 0) and in lower half
					elif direction > 0 and centroid[1] > (H // 2) and min_y <= (H // 2 + 20):
						totalUp += 1
						date_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
